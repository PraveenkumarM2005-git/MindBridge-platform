...
// Decode a JWT without verifying signature (safe for client-side use)
...
const App = () => {
  ...
  // Prevent double fetchProfile calls between checkAuth and onAuthStateChange
  ...
  useEffect(() => {
    ...
    // Safety net: never hang on loading screen more than 4 seconds total
    ...
    const init = async () => {
      try {
        // ── Step 1: Check URL hash for OAuth implicit flow tokens ──
        ...
        if (hash && hash.includes('access_token=')) {
          ...
          if (accessToken) {
            ...
            if (jwt?.sub) {
              ...
              // Try to register session with Supabase (non-blocking — clock skew may reject)
              supabase.auth.setSession({ access_token: accessToken, refresh_token: refreshToken })
                .catch(() => {/* ignore clock skew errors */});

              const userSession = { user: { id: jwt.sub, email: jwt.email } };
              if (mounted.current) setSession(userSession);
              profileFetched.current = true;
              await fetchProfile(jwt.sub);
        ...
        // ── Step 2: Check existing persisted session ──
        const { data: { session: existingSession } } = await supabase.auth.getSession();
        if (existingSession?.user) {
          if (mounted.current) setSession(existingSession);
          profileFetched.current = true;
        ...
        // ── Step 3: No session found — show landing ──
        clearTimeout(safetyTimer);
        if (mounted.current) setLoading(false);
    ...
    // Listen for future auth state changes (e.g. magic link callback)
  ...
  const fetchProfile = async (userId) => {
    try {
      ...
      if (data?.role) {
        ...
        // No role = new user, send to onboarding
        setScreen('onboarding');
      }
    } catch (err) {
      console.error('Profile fetch error:', err);
      if (mounted.current) setScreen('onboarding');
    }
  };
    ...
    return (
      <div className="min-h-screen bg-linen flex flex-col items-center justify-center gap-4">
        <div className="relative">
          <div className="w-16 h-16 bg-banyan-green rounded-[1.5rem] flex items-center justify-center shadow-2xl shadow-banyan-green/30">
            <Sparkles className="text-sage" size={32} />
          </div>
          <div className="absolute inset-0 bg-banyan-green/20 rounded-[1.5rem] animate-ping" />
        </div>
        <p className="text-muted-clay font-bold text-sm uppercase tracking-widest animate-pulse">Loading MindBridge...</p>
      </div>
    );
  }
      ...
      <div className="bg-white p-10 md:p-12 rounded-[4rem] border border-banyan-green/5 shadow-xl flex flex-col">
        ...
        <div className="space-y-6 flex-1">
          <div className="flex justify-between items-center border-b border-banyan-green/5 pb-6">
            <div><div className="font-bold text-banyan-green text-xl mb-1">Guided Meera Sharma</div><div className="text-xs text-sage font-black uppercase tracking-widest">Heritage Hub Mentorship</div></div>
            <div className="text-3xl font-black text-sage">+10</div>
          </div>
          <div className="flex justify-between items-center border-b border-banyan-green/5 pb-6">
            <div><div className="font-bold text-banyan-green text-xl mb-1">Learned from Rahul</div><div className="text-xs text-muted-clay font-black uppercase tracking-widest">Digital Literacy Session</div></div>
            <div className="text-3xl font-black text-terracotta">-5</div>
          </div>
          <div className="flex justify-between items-center pb-2">
            <div><div className="font-bold text-banyan-green text-xl mb-1">Welcome Bonus</div><div className="text-xs text-sage font-black uppercase tracking-widest">MindBridge Onboarding</div></div>
      ...
      <div className="px-12 md:px-20 pb-20 -mt-24 relative z-10">
        <div className="flex flex-col md:flex-row justify-between items-end gap-10 mb-16">
          <div className="w-48 h-48 rounded-[3.5rem] border-[12px] border-white overflow-hidden shadow-2xl relative group bg-linen flex items-center justify-center">
            {profile?.full_name ? (
              <span className="text-7xl font-heading font-black text-banyan-green uppercase drop-shadow-sm">{profile.full_name.charAt(0)}</span>
            ) : (
    ...
    <div className="min-h-screen bg-linen font-sans selection:bg-terracotta/20">
      ...
      {message.text && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[9999] max-w-lg w-full px-4">
          <div className={`p-4 rounded-2xl font-bold text-center shadow-2xl flex items-center gap-3 justify-center ${message.type === 'error' ? 'bg-red-600 text-white' : 'bg-emerald-600 text-white'}`}>
            {message.type === 'error' && <AlertCircle size={20} />}
            {message.text}
            <button onClick={() => setMessage({ type: '', text: '' })} className="ml-auto opacity-70 hover:opacity-100">✕</button>
          </div>
        </div>
      )}

      <AnimatePresence mode="wait">
        {screen === 'landing' && <LandingPage key="landing" onStart={() => setScreen('login')} />}

        {screen === 'login' && (
          <div key="login" className="min-h-screen bg-linen flex items-center justify-center p-8 relative overflow-hidden">
        ...
        {screen === 'matching' && <MatchScreen key="matching" role={profile?.role} onAccept={() => setActiveTab('home')} />}
        {screen === 'guardian' && <VoiceGuardian key="guardian" onBack={() => setScreen('dashboard')} />}
      </AnimatePresence>
    </div>
  );
};
